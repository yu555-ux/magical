...
export default function AetherModal({ isOpen, onClose, title, children, zIndex = 140, maxWidth = '780px', icon }: Props) {
  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 bottom-[calc(3.5rem+env(safe-area-inset-bottom,0px))] md:bottom-0 flex items-center justify-center p-3 md:p-4" style={{ zIndex }}>
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
        ...
        <motion.div
          ...
          className="relative w-[88vw] md:w-full max-h-[calc(100vh-3.5rem-1.5rem)] md:max-h-[88vh] glass-panel border-glow overflow-hidden flex flex-col
