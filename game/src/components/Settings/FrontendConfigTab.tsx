...
import { AlignCenter, Type, Bold, Italic, MessageSquare, Minus, Plus, Settings } from 'lucide-react';
...
export default function FrontendConfigTab({ draft, setDraft }: Props) {
  ...
  return (
    <div className="p-5 space-y-6">
      ...
      <section className="bg-aether-dark/30 rounded-lg border border-aether-border/20 p-5">
        <div className="flex items-center gap-3 mb-5">
          <div className="w-10 h-10 rounded-full border-2 border-aether-amber/40 bg-aether-amber/10 flex items-center justify-center flex-shrink-0">
            <Settings size={20} className="text-aether-amber" />
          </div>
          <div>
            <h4 className="text-sm font-display font-semibold text-aether-amber tracking-wide">提示词拼接</h4>
            <p className="text-[10px] text-white/25">控制地图和人物信息宏是否经过代码加工处理</p>
          </div>
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className={`flex items-center gap-3 px-4 py-3 rounded-lg border transition-all duration-200 ${
            (draft.useProcessedMap ?? true)
              ? 'bg-aether-dark/40 border-aether-border/15 hover:border-aether-border/30'
              : 'bg-aether-dark/20 border-white/[0.04] opacity-70'
          }`}>
            <button
              onClick={() => setDraft({ ...draft, useProcessedMap: !(draft.useProcessedMap ?? true) })}
              className={`relative w-9 h-5 rounded-full transition-colors duration-200 flex-shrink-0 ${
                (draft.useProcessedMap ?? true) ? 'bg-aether-cyan/80' : 'bg-white/[0.12]'
              }`}
            >
              <div className={`absolute top-0.5 w-4 h-4 rounded-full bg-white transition-transform duration-200 ${
                (draft.useProcessedMap ?? true) ? 'left-[18px]' : 'left-[2px]'
              }`} />
            </button>
            <div>
              <span className="text-xs text-white/70 font-display tracking-wide">{'{{MAP}} 地图宏'}</span>
              <p className="text-[10px] text-white/25 mt-0.5">
                {(draft.useProcessedMap ?? true) ? '按位置距离过滤后输出' : '直接输出原始地图变量树'}
              </p>
            </div>
          </div>
          ...
          {/* Characters toggle */}
          <div className={`flex items-center gap-3 px-4 py-3 rounded-lg border transition-all duration-200 ${
            (draft.useProcessedCharacters ?? true)
              ? 'bg-aether-dark/40 border-aether-border/15 hover:border-aether-border/30'
              : 'bg-aether-dark/20 border-white/[0.04] opacity-70'
          }`}>
            <button
              onClick={() => setDraft({ ...draft, useProcessedCharacters: !(draft.useProcessedCharacters ?? true) })}
              className={`relative w-9 h-5 rounded-full transition-colors duration-200 flex-shrink-0 ${
                (draft.useProcessedCharacters ?? true) ? 'bg-aether-cyan/80' : 'bg-white/[0.12]'
              }`}
            >
              <div className={`absolute top-0.5 w-4 h-4 rounded-full bg-white transition-transform duration-200 ${
                (draft.useProcessedCharacters ?? true) ? 'left-[18px]' : 'left-[2px]'
              }`} />
            </button>
            <div>
              <span className="text-xs text-white/70 font-display tracking-wide">人物信息宏</span>
              <p className="text-[10px] text-white/25 mt-0.5">
                {(draft.useProcessedCharacters ?? true)
                  ? '按位置/梦境/上下文过滤后输出'
                  : '直接输出原始人物变量树（{{FEMALE_STRANGER}} 等）'}
              </p>
            </div>
          </div>
        </div>
