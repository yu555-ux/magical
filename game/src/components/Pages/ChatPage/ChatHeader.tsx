...
function getDisplayData(vars: Record<string, any> | undefined) {
  ...
  return { time, location, weather, countdown, inDream };
}
    ...
    <div className="px-6 py-3.5 border-b border-aether-cyan/20 bg-aether-deep/90 flex items-center shrink-0 shadow-[0_1px_8px_rgba(0,242,255,0.03)]">
      <div className="flex items-center gap-6">
        <div className="flex items-center gap-2.5">
          ...
          <span className={`font-display text-[13px] tracking-[0.06em] whitespace-nowrap ${display.inDream ? 'text-aether-red/70' : 'text-white/60'}`}>{display.time}</span>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <span className={`font-display text-[13px] tracking-[0.06em] max-w-[200px] truncate ${display.inDream ? 'text-aether-red/70' : 'text-white/65'}`}>{display.location}</span>
        </div>
        <div className="w-px h-4 bg-aether-border/50" />
        <div className="flex items-center gap-2">
          <CloudSun size={14} className={display.inDream ? 'text-aether-red/70' : 'text-aether-blue/70'} />
          <span className={`font-display text-[13px] tracking-[0.06em] max-w-[120px] truncate ${display.inDream ? 'text-aether-red/70' : 'text-white/55'}`}>{display.weather}</span>
        </div>
        <div className="w-px h-4 bg-aether-border/50" />
        <div className="flex items-center gap-2">
          <span className={`text-[10px] font-mono tracking-[0.1em] ${display.inDream ? 'text-aether-red/50' : 'text-white/20'}`}>
            {display.inDream ? '离开倒计时' : '进入倒计时'}
          </span>
          <span className={`font-mono text-[12px] tracking-[0.06em] ${display.inDream ? 'text-aether-red/70' : 'text-white/45'}`}>{display.countdown}</span>
        </div>
      </div>
