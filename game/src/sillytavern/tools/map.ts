...
update_map: {
  ...
  category: 'variable',
  description:
    '管理地图节点：新增子地点、更新描述/探索信息、管理异常条目。工具自动在地图树中搜索匹配 location。\n\n' +
    '【必须调用的场景】\n' +
    '- 探索到地图中不存在的新区域 → add_child\n' +
    '- 发现某地点的隐藏信息或细节 → add_info\n' +
    '- 地点描述需要更新 → update_desc\n' +
    '- 遭遇/击败/发现新的异常 → add_anomaly\n' +
    '- 异常具现进度变化 → update_anomaly\n' +
    '- 异常被清除 → remove_anomaly\n\n' +
    '【严禁的行为】\n' +
    '- 凭空编造不存在的地点——先用 get_status 查看地图树确认\n' +
    '- 为每个到访地点都 add_child——只记录有意义的新地点\n' +
    '- reason 写成标签（"新增地点"）而非为什么需要这个地点\n\n' +
    '【你的职责】\n' +
    '你不是地图的绘制者，你是探索发现的记录者。\n' +
    '每次地图变更都是玩家探索或世界变化的结果。\n' +
    '✅ "在废弃大楼中发现了隐藏的地下室入口"  ❌ "新增地点"\n\n' +
    '【action 说明】\n' +
